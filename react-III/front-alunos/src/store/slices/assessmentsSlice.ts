...
import { IAssessment, IAssessmentPayload, TAssessmentPayload } from "../../types";
import { setLoading } from "./loadingSlice";
import { RootState } from "..";
import { createAssessments, deleteAssessment, getAssessments, updateAssessment } from "../../services/assessment";
...
export const updateAssessmentThunk = createAsyncThunk('assessments/updateAssessment', async ({id, grade, discipline}: TAssessmentPayload, config) => {
  config.dispatch(setLoading(true))
...
const assessmentsSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(updateAssessmentThunk.fulfilled, (state, action) => {
      return state.map(assessment  => assessment.id === action.payload.id ? action.payload : assessment)
    })
  },
})
